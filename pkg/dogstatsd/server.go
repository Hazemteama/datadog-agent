...
var (
	...
	float64List = newFloat64ListPool()
)
...
func NewServer(aggregator *aggregator.BufferedAggregator) (*Server, error) {
	// TODO: we reset the pool to take into account the telemetry
	// configuration. We should create it once here and once in the tests.
	float64List = newFloat64ListPool()
